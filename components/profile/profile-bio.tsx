...
const ProfileBio = ({ user, userId }: { user: IUser; userId: string }) => {
  ...
  const onUnfollow = async () => {
    try {
      ...
      await axios.delete("/api/follows", {
        data: { userId: user.id, currentUserId: userId },
      });
      router.refresh();
      setIsLoading(false);
    } catch (error) {
  ...
  const openFollowModal = async () => {
    try {
      setOpen(true);
      const data = await getFollowUser(user.id, "following");
      setFollowing(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const onFollowers = async () => {
    ...
    if (followers.length === 0) {
      const data = await getFollowUser(user.id, "followers");
      setFollowers(data);
    }
  };
  ...
  return (
    <>
      <EditModal user={user} />
      <div className="border-b-[1px] border-slate-200 dark:border-neutral-800 pb-4">
